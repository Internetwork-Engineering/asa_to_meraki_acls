...
def get_nested_mx_object(org_id, print_console, content):
    """
    Process nested object from network group content.
    :param org_id: meraki org id
    :param content: content we are processing
    :return:
    """
    global objects
    new_mx_object = {}
    new_mx_object['category'] = 'network'
    new_mx_object['type'] = 'cidr'
    if content[1] == 'host':
        host_name = f"HOST-{content[2].replace('.', '_')}"
        new_mx_object['name'] = host_name
        new_mx_object['cidr'] = content[2] + '/32'
    else:
        net_subnet_mask = SUBNET_MASKS[content[2]]
        net_name = f"NET-{content[1].replace('.', '_')}-{net_subnet_mask}"
        new_mx_object['name'] = net_name
        new_mx_object['cidr'] = content[1] + '/' + net_subnet_mask
    if new_mx_object['name'] in objects:
        print_console.print(f'[green]Found nested object = {new_mx_object['name']}[/]')
        return objects[new_mx_object['name']]
    else:
        nested_object = dashboard.organizations.createOrganizationPolicyObject(
            organizationId=org_id,
            name=new_mx_object['name'],
            category=new_mx_object['category'],
            type=new_mx_object["type"],
            cidr=new_mx_object["cidr"]
        )
        # Add new object to list
        objects[nested_object['name']] = nested_object['id']
        print_console.print(f'[green]Nested object created = {nested_object['name']}[/]')
        return nested_object['id']
        ...
        for line in lines:
            ...
            if content[0] == 'nat':
                ...
                if content[2] == 'static' and name not in nat_table:
                    ...
                    cidr = dashboard.organizations.getOrganizationPolicyObject(
                        organizationId=org_id,
                        policyObjectId=objects[name]
                    )['cidr']
                    internal_ip = cidr.split('/')[0]
                    cidr = dashboard.organizations.getOrganizationPolicyObject(
                        organizationId=org_id,
                        policyObjectId=objects[content[3]]
                    )['cidr']
                ...
                if content[0] == 'network-object':
                    if content[1] == 'object':
                        ...
                        if obj_name not in objects:
                            ...
                            return None
                        object_id = objects[obj_name]
                        mx_object['objectIds'].append(object_id)
                    else:
                        nested_object_id = get_nested_mx_object(org_id, print_console, content)
                        if nested_object_id:
                            mx_object['objectIds'].append(nested_object_id)
                        else:
                            print_console.print('[red]Failed creating nested object... skipping.[/]')
                            return None
                # nested group object case
                elif content[0] == 'group-object':
                    grp_name = content[1].replace('.', '_')
                    if grp_name not in object_groups:
                        return None
                    group_id = object_groups[grp_name]
            ...
            for route in routes:
                ...
                if route['name'] not in existing_routes:
                    dashboard.appliance.createNetworkApplianceStaticRoute(
                        networkId=network_id, name=route['name'],
                        subnet=route['subnet'],
            ...
            for vlan in vlans:
                ...
                if vlan['name'] not in existing_vlans:
                    dashboard.appliance.createNetworkApplianceVlan(
                        networkId=network_id, id=vlan['id'],
